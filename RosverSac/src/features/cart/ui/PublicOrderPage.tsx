...
/** Vista pública del link temporal `/p/:slug` (pedido del carrito). */
export function PublicOrderPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null)
      try {
        const q = await api<PublicOrder>(
          `/api/orders/public/${encodeURIComponent(slug)}`,
        )
              ...
              {data.shipAddress ? (
                <div className="sm:col-span-2">
                  ...
                  <dd className="text-xs text-rosver-muted">
                    {[data.shipDistrict, data.shipProvince, data.shipDepartment]
                      .filter(Boolean)
                      .join(' · ')}
                  </dd>
                </div>
