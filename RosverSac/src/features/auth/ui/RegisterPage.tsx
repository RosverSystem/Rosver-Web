...
export function RegisterPage() {
  ...
  async function handleConfirmPhone() {
    ...
    try {
      ...
      clear()
      showSuccess(['Cuenta creada. Revisa tu correo con el código.'])
    } catch (err) {
      const msg =
  ...
  return (
    <AuthSplitShell
      heroSrc={brandMediaPath(BRAND_KEYS.registerHero)}
      heroCaption="Tu proyecto, nuestra herramienta."
      floating={
        <>
          <FloatingToasts
            toasts={toasts}
    ...
    >
      ...
      <h1 className="font-display text-3xl font-bold tracking-tight text-rosver-ink sm:text-4xl xl:text-5xl [@media(max-height:700px)]:text-3xl">
        {pendingVerifyEmail ? 'Verifica tu correo' : 'Crear cuenta'}
          ...
          <p className="mt-2 text-sm leading-relaxed text-rosver-muted sm:mt-3 sm:text-base [@media(max-height:700px)]:mt-1.5">
            {pendingVerifyEmail
              ? 'Ingresa el código OTP para activar tu cuenta (válido 24 h).'
              : 'Regístrate en minutos. Deberás verificar el correo con OTP en las próximas 24 horas.'}
          </p>

      {pendingVerifyEmail ? (
        <div className="mt-7 sm:mt-8 [@media(max-height:700px)]:mt-5">
          <OtpVerifyPanel
        ...
        <>
          <ul className="mt-4 space-y-2 sm:mt-5 [@media(max-height:780px)]:hidden">
            {[
          ...
          <form
            ...
            className="mt-6 flex flex-col gap-3 sm:mt-8 sm:gap-3.5 [@media(max-height:700px)]:mt-4 [@media(max-height:700px)]:gap-2.5"
          >
          ...
          <div className="mt-6 flex items-center justify-center gap-3 sm:mt-7 [@media(max-height:700px)]:mt-4">
            <a
              href={googleStartUrl}
          ...
          <p className="mt-6 text-center text-sm text-rosver-muted sm:mt-8 [@media(max-height:700px)]:mt-4">
            ¿Ya tienes cuenta?{' '}
            <Link
              to="/login"
          ...
          <p className="mt-3 text-center text-xs text-rosver-muted sm:mt-4">
